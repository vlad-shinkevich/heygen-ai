...
- 📊 История генераций с отслеживанием в базе данных
- 📤 Автоматическая отправка готовых видео в Telegram
- 💳 Отслеживание расхода кредитов HeyGen
...
| `/api/video/history` | GET | История генераций пользователя |
| `/api/video/credits` | GET | Статистика кредитов пользователя |
| `/api/video/process-completed` | POST | Обработка завершенных видео (cron) |
| `/api/webhook/heygen` | POST | Webhook для уведомлений от HeyGen |
...
После завершения генерации видео автоматически отправляются пользователю в Telegram. 

**Настройка:**
1. **Webhook (рекомендуется):** Настройте в HeyGen Dashboard → Webhooks → `https://your-app.vercel.app/api/webhook/heygen`
2. **Cron Job:** Автоматически настроен через `vercel.json` (проверка каждые 2 минуты)
...
- Расход кредитов на каждую генерацию
- Общий расход кредитов пользователя
- Статистику генераций (успешные/неудачные)

Данные хранятся в таблицах:
- `video_generations` - история каждой генерации
- `credit_tracking` - общая статистика по пользователям
